...
void InitLoadingScreens(String file) {
    ...
    while (!bbEof(f)) {
        ...
        if (bbLeft(TemporaryString,1) == "[") {
            ...
            for (i = 0; i <= 4; i++) {
                ls->txt[i] = GetINIString(file, TemporaryString, "text"+Str(i+1));
                if (ls->txt[i]!= "") {
...
void DrawLoading(int percent, int shortloading = false) {
    ...
    bbColor(0,0,0);
    bbText(userOptions->screenWidth / 2 + 1, userOptions->screenHeight / 2 - 100 + 1, "LOADING - " + Str(percent) + " %", true, true);
    bbColor(255,255,255);
    bbText(userOptions->screenWidth / 2, userOptions->screenHeight / 2 - 100, "LOADING - " + Str(percent) + " %", true, true);
