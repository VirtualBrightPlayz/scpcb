...
void DrawLoading(int percent, int shortloading) {
    ...
    do {
        ...
        bbColor(0,0,0);
        bbText(userOptions->screenWidth / 2 + 1, userOptions->screenHeight / 2 - 100 + 1, txtMgmt->getLocalTxt("mnu_loading") + " - " + String(percent) + " %", true, true);
        bbColor(255,255,255);
        bbText(userOptions->screenWidth / 2, userOptions->screenHeight / 2 - 100, txtMgmt->getLocalTxt("mnu_loading") + " - " + String(percent) + " %", true, true);

        if (percent == 100) {
            //If (firstloop And SelectedLoadingScreen\title <> "CWM") Then PlaySound2(HorrorSFX(8) ;TODO: fix)
            timing->prevTime = bbMilliSecs();
            bbText(userOptions->screenWidth / 2, userOptions->screenHeight - 50, txtMgmt->getLocalTxt("mnu_loaddone"), true, true);
        } else {
            bbFlushKeys();
            bbFlushMouse();
        }
