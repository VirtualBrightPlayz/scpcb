...
void FillRoom_cont_079_1(Room* r) {
    ...
    bbScaleEntity(r->objects[0], 1.3f, 1.3f, 1.3f, true);
    ...
    bbPositionEntity(r->objects[1], 0.082f, 0.119f, 0.010f);
    bbScaleSprite((Sprite*)r->objects[1],0.18f*0.5f,0.145f*0.5f);
    bbTurnEntity((Sprite*)r->objects[1],0,13.f,0);
    bbMoveEntity(r->objects[1], 0,0,-0.022f);
...
void UpdateEvent_cont_079_1(Event* e) {
    if (mainPlayer->currRoom == e->room) {
        ...
        if (e->eventState == 0) {
            ...
            e->room->npc[0] = CreateNPC(NPCtypeGuard, bbEntityX(e->room->objects[2],true), bbEntityY(e->room->objects[2],true)+0.5f, bbEntityZ(e->room->objects[2],true));
        ...
        if (RemoteDoorOn) {
            ...
            if (e->eventState == 1) {
                ...
                if (e->eventState < 3500) {
                    ...
                    if (bbEntityDistance(e->room->objects[0], mainPlayer->collider)<2.5f) {
                        e->eventState = 10001;
