...
void FillRoom_srvr_farm_3(Room* r) {
    ...
    bbPositionEntity(r->objects[0], r->x + 736.0 * RoomScale, -512.0 * RoomScale, r->z - 400.0 * RoomScale, true);
    r->objects[1] = bbCreatePivot(r->obj);
    bbPositionEntity(r->objects[1], r->x - 552.0 * RoomScale, -512.0 * RoomScale, r->z - 528.0 * RoomScale, true);
    r->objects[2] = bbCreatePivot(r->obj);
    bbPositionEntity(r->objects[2], r->x + 736.0 * RoomScale, -512.0 * RoomScale, r->z + 272.0 * RoomScale, true);
...
void UpdateEvent_srvr_farm_3(Event* e) {
    ...
    if (mainPlayer->currRoom == e->room) {
        ...
        if (e->room->objects[3]>0) {
            ...
            bbPositionEntity(e->room->objects[3], bbEntityX(e->room->objects[3],true), (-608.0*RoomScale)+0.05+bbSin(e->eventState+270)*0.05, bbEntityZ(e->room->objects[3],true), true);
        }
    }
    //[End Block]
}
