...
void FillRoom_closets_2(Room* r) {
    ...
    Item* clipboard = CreateItem("clipboard",r->x + 736.0 * RoomScale, r->y + 224.0 * RoomScale, r->z -480.0 * RoomScale);
    bbEntityParent(it->collider, r->obj);
    ...
    d->open = false;
    d->autoClose = false;
    bbMoveEntity(d->buttons[0], 0.0, 0.0, 22.0 * RoomScale);
    bbMoveEntity(d->buttons[1], 0.0, 0.0, 22.0 * RoomScale);
...
void UpdateEvent_closets_2(Event* e) {
    ...
    if (e->eventState == 0) {
        ...
        if (e->eventState < 70*3.5) {
            bbRotateEntity(e->room->npc[1]->collider,0,CurveAngle(e->room->angle+90,bbEntityYaw(e->room->npc[1]->collider),100.0),0,true);
            ...
            if (e->eventState > 70*4.5) {
                ...
                bbRotateEntity(e->room->npc[0]->collider,0,CurveAngle(bbEntityYaw(e->room->npc[0]->obj),bbEntityYaw(e->room->npc[0]->collider),30.0),0,true);
            }
            bbPointEntity(e->room->npc[1]->obj, e->room->obj);
            bbTurnEntity(e->room->npc[1]->obj, 0, bbSin(e->eventState)*25, 0);
            bbRotateEntity(e->room->npc[1]->collider,0,CurveAngle(bbEntityYaw(e->room->npc[1]->obj),bbEntityYaw(e->room->npc[1]->collider),30.0),0,true);
        } else {
            if (e->eventState-timing->tickDuration < 70*6.5) {
                PlaySound_SM(sndManager->lightSwitch);
            }
            mainPlayer->blinkTimer = Max((70*6.5-e->eventState)/5.0 - bbRnd(0.0,2.0),-10);
