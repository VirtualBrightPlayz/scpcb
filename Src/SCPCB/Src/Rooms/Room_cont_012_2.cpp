...
void UpdateEvent_cont_012_2(Event* e) {
    ...
    if (mainPlayer->currRoom == e->room) {
        ...
        if (e->eventState==0) {
            if (bbEntityDistance(mainPlayer->collider, e->room->doors[0]->obj)<2.5 & RemoteDoorOn) {
                ...
                if (temp==false) {
                    ...
                    if (dist < 0.6) {
                        ...
                        if (e->eventState3>70 & e->eventState3-timing->tickDuration<=70) {
                            ...
                            de = CreateDecal(DECAL_BLOOD_POOL,  bbEntityX(mainPlayer->collider), -768*RoomScale+0.01, bbEntityZ(mainPlayer->collider),90,bbRnd(360),0);
                            de->size = 0.1;
                            de->maxSize = 0.45;
