...
void FillRoom_tnnl_maintenance_2(Room* r) {
    ...
    r->doors[0]->open = true;
    bbPositionEntity(r->doors[0]->buttons[1], r->x + 224.f * RoomScale, 0.7f, r->z + 480.f * RoomScale, true);
    bbPositionEntity(r->doors[0]->buttons[0], r->x + 304.f * RoomScale, 0.7f, r->z + 832.f * RoomScale, true);

    r->doors[2] = CreateDoor(r->x - 264.f * RoomScale, 0.f, r->z - 656.f * RoomScale, 90, r, true);
    r->doors[2]->autoClose = false;
    r->doors[2]->open = true;
    bbPositionEntity(r->doors[2]->buttons[0], r->x - 224.f * RoomScale, 0.7f, r->z - 480.f * RoomScale, true);
    bbPositionEntity(r->doors[2]->buttons[1], r->x - 304.f * RoomScale, 0.7f, r->z - 832.f * RoomScale, true);
    //
