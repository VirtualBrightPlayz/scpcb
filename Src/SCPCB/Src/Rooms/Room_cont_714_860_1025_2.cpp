...
void FillRoom_cont_714_860_1025_2(Room* r) {
    Door* d = CreateDoor(r->x + 264.f * RoomScale, 0, r->z, 90, r, true, DOOR_TYPE_DEF, r->roomTemplate->name);
    d->autoClose = false;
    d->open = false;
    ...
    Item* it = CreateItem("scp420j", r->x - 552.f * RoomScale, r->y + 220.f * RoomScale, r->z - 728.f * RoomScale);
    bbEntityParent(it->collider, r->obj);

    it = CreateItem("scp860", r->x + 568.f * RoomScale, r->y + 178.f * RoomScale, r->z + 760.f * RoomScale);
    bbEntityParent(it->collider, r->obj);

    SecurityCam* sc = CreateSecurityCam(r->x + 560.f * RoomScale, r->y + 386 * RoomScale, r->z - 416.f * RoomScale, r);
    sc->angle = 180;
    sc->turn = 30;
