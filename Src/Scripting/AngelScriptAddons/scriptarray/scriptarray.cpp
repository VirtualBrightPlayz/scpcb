...
static std::map<asIScriptEngine*, std::vector<asITypeInfo*>> knownArrayTypes;
...
static bool ScriptArrayTemplateCallback(asITypeInfo* ti, bool &dontGarbageCollect)
{
    ...
    if (knownArrayTypes.find(ti->GetEngine()) == knownArrayTypes.end()) {
        knownArrayTypes.emplace(ti->GetEngine(), std::vector<asITypeInfo*>());
    }
    knownArrayTypes[ti->GetEngine()].push_back(ti);
...
bool IsScriptArrayType(asITypeInfo* typeInfo)
{
    if (knownArrayTypes.find(typeInfo->GetEngine()) == knownArrayTypes.end()) { return false; }
    for (int i = 0; i < knownArrayTypes[typeInfo->GetEngine()].size(); i++)
    {
        if (knownArrayTypes[typeInfo->GetEngine()][i] == typeInfo) { return true; }
    }
    return false;
}
