...
PGE::Vector3f Collider::applyGravity(PGE::Vector3f position, float horizontalSpeed, float timeStep) {
    ...
    while (true) {
        ...
        if (coll.hit) {
            PGE::Vector3f resultPos = line.pointA.add(line.pointB.subtract(line.pointA).multiply(coll.coveredAmount * 0.995f));
            if (iterations == 0) {
                line = PGE::Line3f(position, position.add(PGE::Vector3f(0.f, -fallSpeed, 0.f)));
            }
            ...
            if (abs(coll.normal.y) > 0.71f && abs(coll.normal.y - coll.surfaceNormal.y) < 0.05f) {
                ...
                PGE::Vector3f reflectedDir = line.pointB.subtract(line.pointA).reflect(coll.normal).multiply(-remainingDist);
...
PGE::Vector3f Collider::tryMove(PGE::Vector3f position, const PGE::Vector3f& goal) const {
    ...
    PGE::Vector3f targetDir = line.pointB.subtract(line.pointA).normalize();
    ...
    while (true) {
        ...
        if (coll.hit) {
            ...
            PGE::Vector3f reflectedDir = currDir.reflect(coll.normal).multiply(-remainingDist);
            PGE::Plane p = PGE::Plane(coll.normal, position);
            PGE::Vector3f tempPos = position;
            tempPos = tempPos.add(reflectedDir);
            tempPos = tempPos.subtract(p.normal.multiply((tempPos.subtract(position)).dotProduct(p.normal) * 0.995f));
