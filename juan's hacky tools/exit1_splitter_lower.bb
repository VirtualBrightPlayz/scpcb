...
Function LoadRMesh(file$)
	...
	For i=1 To count ;drawn mesh
		...
		For j=0 To 1
			...
			If temp1i<>0 Then
				temp1s=ReadString(f)
				;temp1s = Replace(temp1s,"exit_gateb_1","extend_gateb")
		...
		Local vertexIndices.IntArrayList = CreateIntArrayList()
		;Local triangleIndices.IntArrayList = CreateIntArrayList()
		...
		For j%=1 To count2
			...
			temp3i=ReadByte(f)
			If vertex>=0 Then
				temp1 = temp1i : temp1 = Max(temp1-48,0) : temp1 = Min(255,temp1*(255.0/207.0)
				temp2 = temp2i : temp2 = Max(temp2-48,0) : temp2 = Min(255,temp2*(255.0/207.0))
				temp3 = temp3i : temp3 = Max(temp3-48,0) : temp3 = Min(255,temp3*(255.0/207.0))
				VertexColor surf,vertex,temp1,temp2,temp3,1.0
			EndIf
		Next
		
		count2=ReadInt(f) ;polys
		Local aa%=-1,bb%=-1,cc%=-1
		For j%=1 To count2
			...
			If surf<>0 Then
				...
				bb = -1
				cc = -1
				For k% = 0 To vertexIndices\size-1
					If GetIntArrayListElem(vertexIndices,k)=temp1i Then
						aa = k
					EndIf
					If GetIntArrayListElem(vertexIndices,k)=temp2i Then
						bb = k
					EndIf
					If GetIntArrayListElem(vertexIndices,k)=temp3i Then
						cc = k
					EndIf
					If aa>=0 And bb>=0 And cc>=0 Then Exit
				Next
				If aa>=0 And bb>=0 And cc>=0 Then
					AddTriangle(surf,aa,bb,cc)
				EndIf
			EndIf
		Next
		
		DeleteIntArrayList(vertexIndices)
		;DeleteIntArrayList(triangleIndices)
	...
	If count>0 Then
		vertexIndices.IntArrayList = CreateIntArrayList()
		aa = -1 : bb = -1 : cc = -1
		...
		For i%=1 To count
			...
			For j%=1 To count2
				...
				Next
				If aa>=0 And bb>=0 And cc>=0 Then
					AddTriangle(surf,totalVerts+aa,totalVerts+bb,totalVerts+cc)
				EndIf
			Next
		Next
	...
	For i%=1 To count
		...
		Select temp1s
			...
			Case "light"
				...
				If y<4000.0 And (x+y+z<>0.0) Then
					DebugLog "LIGHT "+x+" "+y+" "+z+" "
					...
					WriteByte(rm2,Min(intensity,1.0)*255)
				EndIf
			Case "spotlight"
				x = ReadFloat(f) : y = ReadFloat(f) : z = ReadFloat(f)
				range# = ReadFloat(f)
				...
				If y>4000.0 And (x+y+z<>0.0) Then
					...
					WriteFloat(rm2,innerconeangle) ;innerconeangle
					WriteFloat(rm2,outerconeangle) ;outerconeangle
				EndIf
			Case "soundemitter"
				x = ReadFloat(f) : y = ReadFloat(f) : z = ReadFloat(f)
				temp1i = ReadInt(f)
				...
				If y<4000.0 Then
					...
					WriteFloat(rm2,y)
					WriteFloat(rm2,z)
					WriteByte(rm2,temp1i)
					WriteFloat(rm2,range)
				EndIf
			Case "model"
